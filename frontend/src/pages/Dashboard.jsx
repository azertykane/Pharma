...
export default function Dashboard({ token, onLogout, user }) {
  ...
  const block = async (id) => {
    await axios.post(`/api/machines/${id}/block`, {}, { headers: { Authorization: `Bearer ${token}` } });
    fetchMachines(); fetchLogs();
  };

  const unblock = async (id) => {
    await axios.post(`/api/machines/${id}/unblock`, {}, { headers: { Authorization: `Bearer ${token}` } });
    fetchMachines(); fetchLogs();
  };
